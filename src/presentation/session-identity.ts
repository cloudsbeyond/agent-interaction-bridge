...
const DEFAULT_MAX_IDENTIFIER_LENGTH = 12;
const SESSION_IDENTITY_LINE_RE = /^(?:[-*]\s+)?(?:>\s*)?(?:Session：Bridge\s*-\s*\S+\s*\|\s*Domain\s*-\s*\S+|Bridge Session:\s*\S+\s*\|\s*Domain Session:\s*\S+)$/u;

export function compactSessionIdentifier(
  value: string | undefined,
  maxLength = DEFAULT_MAX_IDENTIFIER_LENGTH,
): string {
...
export function renderSessionIdentityPlainText(identity: InteractionSessionIdentity): string {
  return `Session：Bridge - ${compactSessionIdentifier(identity.bridge)} | Domain - ${compactSessionIdentifier(identity.domain)}`;
}
