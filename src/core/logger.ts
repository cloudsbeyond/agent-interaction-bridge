 ...
 *  1. JSON line into the bridge data dir's `logs/YYYY-MM-DD.log` — the durable
 ...
 * the local machine — specifically, before /doctor feeds it to Codex and
 * before the analysis card lands in a Feishu chat.
 *
...
export function sanitizeLogsForDoctor(logs: string): string {
  ...
  out = out.replace(
    /"(sessionId|threadId)":"([^"]{8,})"/g,
    (_, key: string, val: string) => `"${key}":"…${val.slice(-6)}"`,
  );
  out = out.replace(
    /"(cwd|configPath)":"([^"]+)"/g,
    (_, key: string, val: string) => `"${key}":"${sanitizePathValue(val)}"`,
  );
