...
/**
 * Bridge runtime secret resolver. Accepts a small provider-neutral
 * `ResolveSecretInput` contract so operators can keep their App Secret out of
 * `config.json` via:
 *
 *   - plain string                              → as-is
 ...
 * start` working without the bridge CLI on $PATH).
 */
 ...
 * `ref.id`. Exec providers receive a JSON request on stdin and return a JSON
 * response on stdout, so operators can write one small resolver script and
 * point multiple local tools at it.
 *
 * If the configured command IS this same bridge binary (a.k.a. bridge is
 * self-hosting via `agent-interaction-bridge secrets get`), short-circuit and
