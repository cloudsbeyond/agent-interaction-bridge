 ...
 * first-run setup path. Preserves the existing `preferences` block so
 * users don't lose unrelated settings on credential update.
 *
 * The provider command is a thin shell wrapper bridge writes under
 * the bridge data dir's `secrets-getter` (always user-owned, never a symlink) so
...
export async function ensureSecretsGetterWrapper(): Promise<string> {
  ...
  const content =
    `#!/bin/sh\n` +
    `# Auto-generated by Agent-Interaction-Bridge. Do not edit.\n` +
