...
export async function handleCommentMention(deps: CommentDeps): Promise<void> {
  ...
  // One agent runtime session per cloud-doc; subsequent @-mentions in the same
  // doc continue the same conversation. Endpoint profile policy is applied
  // before session lookup so guest/team profiles cannot inherit host state.
  const synthChatId = `doc:${evt.fileToken}`;
  const requestedCwd = workspaces.cwdFor(synthChatId) ?? defaultAgentTaskCwd({ agent, cfg });
  const profilePlan = await prepareAgentProfileRunPlan({
    profileId: getAgentEndpointProfileId(cfg),
  ...
  const cwd = profilePlan.run.cwd ?? requestedCwd;
  const sessionContextVersion = agentSessionContextVersion('adapter');
  const resumeFrom = sessions.resumeFor(
    synthChatId,
    cwd,
    profilePlan.agentRuntimeId,
  ...
  log.info('comment', 'session', {
    ...
    cwd,
    agentRuntimeId: profilePlan.agentRuntimeId,
    endpointProfileId: profilePlan.profile.id,
  });
  if (profilePlan.notes.length > 0) {
    log.info('agent-profile', 'comment-policy', {
      scope: synthChatId,
      profile: profilePlan.profile.id,
      notes: profilePlan.notes,
    });
  }
