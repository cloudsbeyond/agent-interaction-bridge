...
/**
 * Pretty-print the list of running bridge processes.
 *
...
export function runPs(): void {
  ...
  const rows = live.map((e, idx) => {
    ...
    return {
      ...
      started: ago,
      version: e.version,
      endpoint: e.agentEndpoint ?? 'unknown',
    };
  });
  const headers = { idx: '#', id: 'ID', pid: 'PID', app: 'Bot', started: '启动', version: '版本', endpoint: 'Endpoint' };
  printTable([headers, ...rows]);
}

export async function runStopCli(target: string | undefined): Promise<void> {
  if (!target) {
    console.error('用法: agent-interaction-bridge stop <bot id 或序号>');
    process.exit(1);
  }
  const entry = resolveTarget(target);
  if (!entry) {
    console.error(`✗ 没找到匹配的 bot:${target}`);
    console.error('  用 `agent-interaction-bridge ps` 看可选目标。');
    process.exit(1);
  }
