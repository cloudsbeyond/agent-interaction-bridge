...
export async function runStart(opts: StartOptions): Promise<void> {
  ...
  if (isComplete(existing)) {
    cfg = existing;
    // If config contains a raw string in accounts.app.secret, move it into
    ...
    cfg = await maybeEncryptPlaintextSecret(cfg, configPath);
  } else {
  ...
  let endpoint = opts.agentEndpoint ?? getAgentEndpointKind(cfg);
  let agent = createAgentAdapter(endpoint, {
    appServerCwd: endpoint === 'app-server' ? getAppServerCwd(cfg) : undefined,
  });
  if (!(await agent.isAvailable())) {
    console.error(`✗ 未找到可用于 ${endpoint} 的 Codex。请先安装并登录 Codex：`);
    console.error('  codex login');
    process.exit(1);
  }
  log.info('agent', 'endpoint-selected', { endpoint, adapter: agent.id });
  ...
  const controls: Controls = {
    ...
    async restart() {
      ...
      try {
        ...
        const nextAgent = createAgentAdapter(nextEndpoint, {
          appServerCwd: nextEndpoint === 'app-server' ? getAppServerCwd(next) : undefined,
        });
...
async function resolveConflict(
  ...
  if (!process.stdin.isTTY) {
    console.warn(
      '⚠️  当前不是交互式启动,已自动取消。如需替换,先用 `agent-interaction-bridge stop <bot id>` 关掉旧的。\n',
    );
    return false;
  }
 ...
 */
async function maybeEncryptPlaintextSecret(
  cfg: AppConfig,
  configPath: string,
): Promise<AppConfig> {
  ...
  if (typeof s === 'string' && !/^\$\{[A-Z][A-Z0-9_]*\}$/.test(s)) {
    try {
      ...
      console.log('🔒 已把 App Secret 加密保存到当前 bridge 数据目录的 secrets.enc');
      return next;
    } catch (err) {
      log.warn('config', 'encrypt-secret-failed', {
        err: err instanceof Error ? err.message : String(err),
      });
      // Encryption failure isn't fatal — runtime resolver still handles the
      // plaintext path.
      return cfg;
    }
  }
  ...
  //   2. The config's `secrets.providers.bridge` block. Rewrite if out of date
  //      so lark-cli's audit sees a user-owned, non-symlinked command path.
  try {
