      ...
      elements: [
        {
          ...
          content:
            '⚙️ **偏好设置**\n\n' +
            '调整 bot 的行为偏好。改完点提交,**立即生效**(无需重启)并写入 bridge 数据目录的 `config.json`。',
        },
        { tag: 'hr' },
        {
          tag: 'form',
          ...
          elements: [
            {
              ...
              content:
                '**消息回复方式**\n' +
                '_纯文本(默认):agent 跑完一次性发出,不流式,可读性最稳_\n' +
                '_消息卡片:轻量流式 markdown 卡片,飞书原生打字机动画,适合看过程_',
            },
            {
              tag: 'select_static',
              name: 'message_reply',
              // 'card' (交互卡片) is hidden from the picker for now; the form
              // displays it as 'markdown' and only overwrites when submitted.
              initial_option: opts.messageReply === 'card' ? 'markdown' : opts.messageReply,
              options: [
                { text: { tag: 'plain_text', content: '纯文本(默认)' }, value: 'text' },
                { text: { tag: 'plain_text', content: '消息卡片(流式)' }, value: 'markdown' },
              ],
            },
            {
              tag: 'markdown',
            ...
            },
            {
              ...
              content:
                '\n**审批与可视化策略**\n' +
                '_默认:普通消息直接执行。用 `/approve 任务` 强制审批,`/run 任务` 直接执行,`/visual 任务` 强制交互卡片,`/quiet 任务` 只发最终文本,`/model <模型> 任务` 指定模型。_',
            },
            {
              tag: 'select_static',
              name: 'require_approval_before_run',
              initial_option: opts.requireApprovalBeforeRun ? 'yes' : 'no',
              options: [
                { text: { tag: 'plain_text', content: '普通消息也先审批' }, value: 'yes' },
                { text: { tag: 'plain_text', content: '仅命令/模型触发审批(默认)' }, value: 'no' },
              ],
            },
            {
              tag: 'input',
              name: 'approval_keywords',
              default_value: opts.approvalKeywords,
              placeholder: { tag: 'plain_text', content: '/approve, /审批, /review' },
              input_type: 'text',
            },
            {
              tag: 'input',
              name: 'approval_models',
              default_value: opts.approvalModels,
              placeholder: { tag: 'plain_text', content: 'gpt-5.5, gpt-5.4（留空=不按模型审批）' },
              input_type: 'text',
            },
            { tag: 'hr' },
            {
            ...
            },
            {
              ...
              content:
                ...
                '_open_id 可从 bridge 日志 `logs/*.log` 里 grep `senderId` 字段_',
            },
            {
              tag: 'input',
              name: 'allowed_users',
      ...
      elements: [
        {
          ...
          content:
            ...
            '🧭 **审批与可视化**\n' +
            `**普通消息审批**:\`${opts.requireApprovalBeforeRun ? '是' : '否'}\`\n` +
            `**审批命令**:${summarizeList(opts.approvalKeywords)}\n` +
            `**审批模型**:${summarizeList(opts.approvalModels)}\n\n` +
            '🔒 **访问控制**\n' +
