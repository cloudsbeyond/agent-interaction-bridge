...
import { normalizeChatPresentation } from './chat-presentation-contract';
import { toolHeaderText } from './tool-render';

/**
 * Render `RunState` as plain markdown text — used in `messageReply: 'text'`
 * mode where we send one final markdown post instead of a streaming card.
 *
...
export function renderText(state: RunState): string {
  ...
  if (state.terminal === 'interrupted') {
    ...
    // errorMsg 已经由 adapter 侧渲染成「带处理建议的」面向用户文案，
    // 这里直接透传，不再前置 “⚠️ agent 失败:” 这种内部术语。
    parts.push(state.errorMsg);
...
function renderBlock(block: Block): string {
  if (block.kind === 'text') {
    return normalizeChatPresentation(block.content);
  }
