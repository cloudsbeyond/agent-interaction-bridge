...
import { displayHomeRelativePath, truncateText } from './card-text';
...
export function toolBodyMd(tool: ToolEntry): string {
  ...
  if (tool.output) {
    const truncated = truncateText(tool.output, OUTPUT_MAX);
  ...
  return `${truncateText(body, BODY_TOTAL_MAX)}\n\n_（body 已截断,完整内容查 \`/doctor\` 或日志）_`;
}
...
function renderInput(tool: ToolEntry): string {
  ...
  switch (tool.name) {
    case 'Bash': {
      ...
      return cmd ? `**Command**\n\`\`\`bash\n${truncateText(cmd, BODY_FIELD_MAX)}\n\`\`\`` : '';
    }
    case 'Read':
    case 'Edit':
    case 'Write':
    ...
    case 'WebSearch':
      return str('query') ? `**Query** \`${truncateText(str('query'), BODY_FIELD_MAX)}\`` : '';
    default:
      return '';
  }
}
...
function shortenPath(p: string): string {
  if (!p) return p;
  return displayHomeRelativePath(p, process.env.HOME || '');
}
