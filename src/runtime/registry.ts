...
import { chmodSync, mkdirSync, readFileSync, renameSync, writeFileSync, unlinkSync } from 'node:fs';
import { chmod, mkdir, rename, writeFile } from 'node:fs/promises';
import { dirname } from 'node:path';
import { paths } from '../config/paths';
import type { AgentEndpointKind, TenantBrand } from '../config/schema';

/**
 * Tracks running bridge start processes so we can:
...
export async function register(args: RegisterArgs): Promise<ProcessEntry> {
  ...
  const entry: ProcessEntry = {
    ...
    ...(args.agentEndpoint ? { agentEndpoint: args.agentEndpoint } : {}),
  };
...
export async function updateEntry(
  id: string,
  patch: Partial<Pick<ProcessEntry, 'appId' | 'tenant' | 'configPath' | 'botName' | 'agentEndpoint'>>,
): Promise<void> {
  const live = readAndPrune();
  let changed = false;
